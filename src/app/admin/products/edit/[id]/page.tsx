...
export default function AdminEditProductPage() {
  ...
  const existingProduct = PRODUCTS.find((p) => p.id === productId || p.slug === productId) || PRODUCTS[0];

  const [name, setName] = useState(existingProduct?.name || "");
  const [description, setDescription] = useState(existingProduct?.descriptionEnglish || existingProduct?.shortDesc || "");
  const [imageUrl, setImageUrl] = useState(existingProduct?.image || "");
  const [deliveryMethod, setDeliveryMethod] = useState<"EMAIL" | "WHATSAPP" | "MESSENGER">(
    existingProduct?.deliveryMethod === "MESSENGER"
      ? "MESSENGER"
      : existingProduct?.deliveryMethod === "WHATSAPP"
      ? "WHATSAPP"
      : "EMAIL"
  );

  const [productType, setProductType] = useState<"SIMPLE" | "VARIANTS">(
    existingProduct?.variations && existingProduct.variations.length > 1 ? "VARIANTS" : "VARIANTS"
  );
  const [simplePrice, setSimplePrice] = useState(String(existingProduct?.minPriceBDT || 290));
  const [simpleComparePrice, setSimpleComparePrice] = useState(String(existingProduct?.maxPriceBDT || 350));
  ...
  const [variants, setVariants] = useState<Array<{
    ...
    existingProduct?.variations?.map((v, i) => ({
      id: v.id || `v-${i}`,
      name: v.name,
      priceBDT: v.priceBDT,
      originalPriceBDT: v.originalPriceBDT || Math.round(v.priceBDT * 1.2),
  ...
  const [category, setCategory] = useState(existingProduct?.category || "AI Tools");
